...
public class BeatController implements ControllerInterface {
	...
	public void decreaseBPM() {
        int bpm = model.getBPM();
        if (bpm - 1 >= 0)	{	//Se agrego para corregir el bug que permit�a setear frecuencias negativas.
			model.setBPM(bpm - 1);
		}
  	}
  
 	public void setBPM(int bpm) {
 		if (bpm >= 0) { 	//Se agrego para corregir el bug que permit�a setear frecuencias negativas.
			model.setBPM(bpm);
		}
	}
